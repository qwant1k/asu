...
import type { PaginatedResponse, UnitOfMeasure } from '../../shared/types';
import {
  C, PageHeader, Btn, Th, Td, Badge, Modal, InputField, Spinner, EmptyState, hoverRow, Popconfirm, Surface, FilterBar,
...
const UnitsOfMeasurePage: React.FC = () => {
  ...
  const [editItem, setEditItem] = useState<UnitOfMeasure | null>(null);
  const [deleteItem, setDeleteItem] = useState<UnitOfMeasure | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse<UnitOfMeasure>>('/references/units-of-measure/', { params });
  ...
  const resetForm = (item?: UnitOfMeasure) => {
    setFName(item?.name || '');
    setFCode(item?.code || '');
  ...
  const openEdit = (r: UnitOfMeasure) => { setEditItem(r); resetForm(r); setModalOpen(true); };

  const handleSave = async () => {
    if (!fName.trim()) { setErrorMsg('Заполните наименование'); return; }
    setSaving(true); setErrorMsg('');
    try {
      const payload = { name: fName.trim(), code: fCode.trim(), is_active: fActive };
      if (editItem) {
        await api.patch(`/references/units-of-measure/${editItem.id}/`, payload);
      } else {
        await api.post('/references/units-of-measure/', payload);
      }
      setModalOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleDelete = async (r: UnitOfMeasure) => {
    try { await api.delete(`/references/units-of-measure/${r.id}/`); setData((prev) => prev.filter((x) => x.id !== r.id)); setTotal((prev) => Math.max(0, prev - 1)); } catch { /* ignore */ }
    setDeleteItem(null);
  };
  ...
  return (
    <div>
      <PageHeader title={t('nav.unitsOfMeasure')} right={<Btn onClick={openCreate}><PlusOutlined /> {t('common.add')}</Btn>} />
