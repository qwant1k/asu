...
import type { PaginatedResponse, Position } from '../../shared/types';
import {
...
const PositionsPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse<Position>>('/references/positions/', { params });
  ...
  const openEdit = (r: Position) => { setEditItem(r); resetForm(r); setModalOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editItem) {
        await api.patch(`/references/positions/${editItem.id}/`, payload);
      } else {
        await api.post('/references/positions/', payload);
      }
      setModalOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleDelete = async (r: Position) => {
    try { await api.delete(`/references/positions/${r.id}/`); setData((prev) => prev.filter((x) => x.id !== r.id)); setTotal((prev) => Math.max(0, prev - 1)); } catch { /* ignore */ }
    setDeleteItem(null);
  };
  ...
  return (
    <div>
      <PageHeader title={t('nav.positions')} right={<Btn onClick={openCreate}><PlusOutlined /> {t('common.add')}</Btn>} />
